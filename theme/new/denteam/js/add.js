jQuery(document).ready(function($) { 
	...
	$(document).on('submit', '#filter_by_term', function(e){
		e.preventDefault();
		let _this = $(this);
		...
		$.ajax({
			...
			success: function(data){
				$('#response_filter_by_term').html(data);
			}
		});
	});

	$(document).on('change', 'input[name="term"]', function(e){
		$('#filter_by_term').submit();
	});
	...
	$(document).on('click', '.more_posts', function(e){
		...
		$.ajax({
			...
			success:function(data){
				if (data) {
					$('#response_more_posts').append(data); 
					this_page++;                      
					if (this_page == _this.attr('data-max-pages')) {
						_this.remove();               
					}
	...
	$(document).on('click', '.load_team', function(e){
		e.preventDefault();
		let _this = $(this);
	...
	$(document).on('click', '.load_about_us', function(e){
		e.preventDefault();
		let _this = $(this);

		let data = {
			'action': 'load_about_us',
			'query': _this.attr('data-param-posts'),
			'page': this_page,
		}
	...
	$(document).on('click', '#multistep_form_submit', function(){
		$( "input[type='radio']:checked" ).each(function(index, element) {
			$('#hidden_' + element.getAttribute('name')).val(element.getAttribute('value'));
		});
	});
